...
const KPICard: React.FC<KPICardProps> = ({ title, value, trend, icon: Icon, color, isDark = true, subtitle }) => {
  return (
    <div className={`p-4 border rounded ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-gray-50 border-gray-300'}`}>
      ...
      <div>
        ...
        {subtitle && (
          <p className={`text-xs mt-1 ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>{subtitle}</p>
        )}
      </div>
    </div>
  );
